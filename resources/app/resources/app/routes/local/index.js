const local = require('express').Router({ mergeParams: true });
...
// Test Route
local.get('/', (req, res) => {
  res.status(200).json({ message: 'Local API' });
});
...
local.post('/ffmpeg/transcode', jsonParser, function (req, res) {
  ...
  const child4 = spawn('ffmpeg', ['-i', `${input}`, '-i', conf.settings.ffmpeg.watermark.sd.wmv, '-filter_complex', '[0:v]scale=854:480[scaled];[scaled][1:v]overlay=0:0', '-b', '2000k', '-vcodec', 'wmv2', '-acodec', 'wmav2', `${input_path}/${basename}_sd.wmv`]);
	child1.on('exit', (code) => {
      ...
      if (code === 0) {
         res.status(200).json({ message: 'FFMpeg/transcode ran successfully.' });
      } else {
        res.status(200).json({ message: `Error Code ${code}`, code: code});
      }
	});
